...
auth.onAuthStateChanged(function(user) {
  if (user) {
    currentUser = user;
    // أولاً: اقرأ بيانات المستخدم من قاعدة البيانات
    db.ref('users/' + user.uid).once('value').then(function(snap) {
      currentUserData = snap.val() || {};

      // إذا لم توجد بيانات (مستخدم جديد) أنشئها من Firebase Auth
      if (!currentUserData.displayName) {
        currentUserData.displayName = user.displayName || user.email.split('@')[0];
      }

      // سجّل الحضور بعد ما البيانات جاهزة
      setPresence(true);
      setupMembersListener();

      // إذا لم تكن البيانات في DB، اكتبها
      if (!snap.exists()) {
...
function setupMembersListener() {
  ...
  membersListener = membersRef.on('value', function(snap) {
    ...
    // حدّث كل العناصر التي تعرض عدد الأعضاء
    ['home-members-count','chat-members-count'].forEach(function(id) {
      var el = document.getElementById(id);
      if (el) el.textContent = id === 'home-members-count' ? count + ' عضو' : count;
    });
...
function updateMicSlots(presenceData) {
  // هذه الدالة لا تُحدّث المايكات بعد الآن
  // setupVoiceListener هي المسؤولة عن المايكات
  // هنا فقط نحدّث العداد في حال احتجنا
  // (تركناها لأنها تُستدعى من setupMembersListener)
}
...
function openChat() {
  ...
  // نحصل على آخر رسالة موجودة لنعرف الـ key الفاصل
  chatRef.orderByChild('timestamp').limitToLast(1).once('value').then(function(snap) {
    var lastKey = null;
    snap.forEach(function(c) { lastKey = c.key; });

    // الآن نستمع لـ child_added — كل رسالة تأتي بعد الـ lastKey
    chatListener = chatRef.orderByKey().on('child_added', function(snap) {
      var key = snap.key;
      var msg = snap.val();
      if (!msg || !msg.timestamp) return;

      // تجاهل الرسائل القديمة (قبل فتح الغرفة)
      if (msg.timestamp < chatOpenedAt - 2000) return;
    ...
    // fallback: استمع للكل وفلتر بالوقت
    chatListener = chatRef.orderByKey().on('child_added', function(snap) {
      var key = snap.key;
      var msg = snap.val();
      if (!msg || !msg.timestamp) return;
...
function sendMessage() {
  ...
  // اعرض الرسالة فوراً على شاشة المرسل (Optimistic UI)
  var newRef = chatRef ? chatRef.push() : db.ref('messages/general').push();
  var key    = newRef.key;
  if (key && !chatSeenKeys[key]) {
    chatSeenKeys[key] = true;   // منع التكرار عند وصولها من Firebase
    appendMessage(msg);
    scrollBottom();
  }
...
function setupVoiceListener() {
  db.ref('voice_mics').on('value', function(snap) {
    // جمّع المستخدمين على كل مايك
    var occupied = {}; // '0','1',... → userData
    snap.forEach(function(slotSnap) {
      // خذ أول مستخدم في هذا المايك (كل مايك يسع شخصاً واحداً)
      var firstUser = null;
      slotSnap.forEach(function(userSnap) {
        if (!firstUser) firstUser = userSnap.val();
      });
      if (firstUser) occupied[slotSnap.key] = firstUser;
    });
    ...
    for (var i = 0; i < 5; i++) {
      ...
      var u      = occupied[String(i)];
      var isMe   = u && currentUser && u.uid === currentUser.uid;

      if (u) {
        // هناك شخص على هذا المايك
        slot.classList.add('occupied');
        slot.style.cssText += ';border-color:' + (isMe ? '#3ecf6a' : 'rgba(124,58,237,0.5)') +
          '!important;background:' + (isMe ? 'rgba(62,207,106,0.18)' : 'rgba(124,58,237,0.12)') + '!important';

        // عرض الصورة أو الحرف الأول
        if (u.photoURL && u.photoURL.length > 10) {
          avDiv.innerHTML = '<img src="' + u.photoURL +
            '" style="width:100%;height:100%;object-fit:cover;border-radius:50%" onerror="this.parentNode.textContent='' +
        ...
        // النص تحت المايك
        if (isMe) {
          lblDiv.textContent = VOICE.muted ? '🔇 كتم' : '🎙️ ' + (u.displayName || 'أنت');
          lblDiv.style.color = '#3ecf6a';
        } else {
        ...
        if (!isMe && VOICE.mySlot !== -1 && u.uid && !VOICE.peers[u.uid]) {
          voiceCallPeer(u.uid);
        }
